...
def validate_book(
    ...
    for i, rule in enumerate(rules):
        ...
        if score < 0.15:
            # ⚠ 第 23 批的实测结论（读 V14 之前请先看这段）：
            # `correspondence` 是「statement 的**汉字集合**有多少落在 quote 里」——是字符集召回，
            # 不是语义判断，也不做分词。statement 大多是**现代概括**（用「必须/不得/条目/篇目」
            # 这类现代词），这些字天然不在古文引文里，于是分数被系统性压低：
            #   · 183 条 V14 里 175 条属这种「概括性」低分；
            #   · 其中 0.000 的 27 条我逐条核过：statement 的实词**一条都不在全文里**
            #     （如「贼克法」条讲的是「重审/元首」、DONGGONGZERI-006 讲的是「四孟之月」），
            #     引文本身逐字正确 —— 低分来自概括，不是引文有问题。
            # 真正该据 V14 动手的只有「quote 形态可疑」那一档（下方判据）。
