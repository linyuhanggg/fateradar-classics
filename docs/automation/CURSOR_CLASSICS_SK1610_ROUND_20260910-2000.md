# cursor-native-classics checkpoint 2026-09-10T20:00Z

Lane: `codex/cursor-classics-autonomy-state`  
Run branch: `cursor/fateradar-classics-processing-4a3e`  
Baseline: `d47f77d` (`origin/codex/grok-fast-corpus-3-20260910`)  
Continued from: `fad173e` / lease `2c095ef`  
...
本轮范围：元理赋「两干不杂」万民英解续——财官生旺逢印至身旺官禄㣲轻。`欄` 起句一段标待核实，不补造格名；三奇赋句止于「自生」，不补阙文。官职/古人名是原书品级语言，不是履历验收。

Lease: ordinary push `2c095ef` acquired; this checkpoint completes the 36 claimed IDs and releases.
...
- 61 段仍是 `原文：` 粘贴，不得称语义完成。
- 补本不能当主本投票；网站章节名不等于印本卷号。
- `𦂳` 按紧读、`䕶`/`㫁`/`㓜`/`䧟`/`㓙`/`䘮`/`夲`/`㣲`/`㸔`/`巺` 不改字。
- `欄` 起句与三奇「自生」截断处不补字。
- 趋艮/趋乾/倒冲/鼠贵/朝阳等只录原文所定日时，不补全表。
