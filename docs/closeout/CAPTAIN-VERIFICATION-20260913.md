...
## 6. captain 对奇门批次2 差异的独立推导（假设，待 t10 验证/推翻）

背景：algo-impl 批次2 结构化复算暴露奇门 1,543 条 `starPalaceRaw` 差异。captain 做了两步：
1. **证伪「恒定值 bug」**：引擎取值分布 1/9/2/8/3/7…（变化），原书取值 3/4/9/7/2/8…（也变化）→ 不是固定返回某一宫，而是系统性错位。
2. **从原表推导量纲**（classics `sources/normalized/san-shi/qimen-dunjia-tongzhi/nlc-layouts.md` P091 阳一局甲子旬）：

| 时支 | 子 | 丑 | 寅 | 卯 | 辰 | 巳 | 午 | 未 | 申 |
|---|---|---|---|---|---|---|---|---|---|
| 原表 | 一一 | 九二 | 八三 | 七四 | 一五 | 二六 | 三七 | 四八 | 五九 |
| 第 2 数 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
| 第 1 数 | 1 | 9 | 8 | 7 | 1 | 2 | 3 | 4 | 5 |

- 第 2 数按时支严格递增 → 只能是**值使门落宫**（休门自值符宫起、阳遁每时顺行一宫），不可能是「原宫」；
- 第 1 数按阳一局地盘（戊1 己2 庚3 辛4 壬5 癸6 丁7 丙8 乙9）把甲子旬时干 甲(戊)乙丙丁戊己庚辛壬癸 映射成宫，恰得 1,9,8,7,1,2,3,4,5 → 是**值符星落宫**（随时干移宫）。

**据此，该页 `table_reading` 中「后两数分别记主星和主门的原宫」的说法很可能不准确**（已登记为 classics 侧待更正的说法，归 classics-audit 范围，captain 未改该文件）。

引擎实测：乙丑时 `starPalaceRaw=1`（原书 9）、丙寅时 `1`（原书 8），而 `doorPalaceRaw` 的 2、3 与原书一致。`1` = 戊宫 = 旬首/值符原宫。因此只剩两种可能，交 t10 用**自造最小输入**判定：
- (a) 引擎 `starPalaceRaw` 语义本就是原宫/旬首宫，**是复算抽取的字段映射错**；或
- (b) 引擎**漏了逐时移宫**（`landing` 未跟时干走）——真引擎缺陷，同时阻塞 A7。

纪律：本条是**假设**，不是结论；不得据它改引擎或改原书期望，必须由 t10 抽样 + 自造输入复现后定论。
