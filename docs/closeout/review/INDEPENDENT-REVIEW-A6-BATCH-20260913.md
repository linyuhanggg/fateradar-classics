...
两条缺陷真实、修复真实有效、三个既有测试**语义保留且覆盖扩大**、导出 token 结构兼容但语义需具名说明、contest-evidence 现场未被触碰；**合 main 前无必修项**，但须同步登记 §7 的 (a)–(e)。

**最终判定：可以合 main。**（本批 `f030441` 范围内无必须修的点；条件是合入前把 (a)–(e) 落进账本 remaining——(a) 五术同类残留 7 处已派 t17，(b)–(e) 为具名小项。若 captain 要求「零残留才合」，则退为「有条件合」，唯一条件即 (a)。）

### captain 四问 → 本文对应位置（便于取证）

| captain 的问题 | 位置 | 判定 |
| --- | --- | --- |
| 1) 六爻：1 点→8 点 / 每点 anchored_text+sourceRevision / 盘级 ZSB-E-09·E-14·E-16·E-17 / 点 ID 不冒充 | §1 + §4 | 通过（8 点是那条调用的值，全量 2–15；8 点中 7 点带锚点，第 8 点为白话总结点） |
