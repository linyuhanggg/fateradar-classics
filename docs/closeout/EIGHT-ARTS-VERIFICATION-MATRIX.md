...
- **奇门**：批次2 结构化复算 1,919 条，抽取口径修正后 **matchedAll 1,842 / withDiff 77**（此前 1,554 条差异系口径未对齐的假差异）。`starPalaceRaw` 口径已证实跟**落宫**走（1,888/1,917 与引擎 raw 落宫一致，仅 376 条与原宫一致）；剩余 77 条差异待逐条分类（t10）。
