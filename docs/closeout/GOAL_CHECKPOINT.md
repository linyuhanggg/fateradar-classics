...
- `source-reviewed` **60,008** / `draft` **2,831**（本轮 2905 → 2831）
  - ⚠️ 此行为**回退前快照**，已被本文件开头「当前真实计数」取代：独立复核后回退 4 条（`1a18545`），现为 `source-reviewed` **60,004** / `draft` **2,835**。60,008−4=60,004、2,831+4=2,835 自洽。回退条目见下「独立复核后的回退」。
...
| 大六壬总钤残格 | **666** | 电子本把表头抽进单元格流，列归属不可复原。**页已定位、格未读出**（见下）；原写「仓内无《大六壬大全》影印」不成立 |
| 非总钤 draft | 107 | 已逐条写缺什么/已查来源/下一步 |
| 影印校勘 | 未做 | 全库 `source-reviewed` 只表示对照电子文本，不等于逐段影印精校 |

### 更正：大六壬《總鈐》影印实际在仓内（2026-09-12 实测）

原先「仓内无《大六壬大全》影印 → 需外部影印页」的判断**不成立**，是照 `source-manifest.yaml` 的
`local_files` 路径得出的错误结论：

- 实测该 manifest 的 **17 条 `local_files` 路径全部不存在**（含 `sources/raw/san-shi/daliuren-daquan/Siku-Wenyuange-0808.djvu`），
  而底本影印其实在仓内另一路径：
  `sources/facsimile/wikisource/files/文淵閣四庫全書 0808冊.djvu`，
  98,547,126 B，已入 git，sha256 `38d40b5c…f816f2db` 与 manifest 自己声明的 hash 一致。
  → manifest 的 `local_files` 路径失准是**根因缺口**，需单独修正，不能据此宣称「已取得影印」或「仓内无影印」。
- 《總鈐》位置已定位：该册《六壬大全》为扫描影像 476–853；**《總鈐》= 影像 480–482（标签页 475–477）= 卷一 葉 4a–8a**，
  交叉印证 Kanripo `KR3g0031_001.txt`（`總鈐` → `001-4a`…`8a` 空行 → `8b` 接 `神煞`）；另有 CADAL 高清四库扫描
  `archive.org/download/06054168.cn/06054168.cn.pdf`（總鈐 = 第 1 册 16–25 页）与独立的清光绪十二年（1886）掃葉山房刻本可互校。
- **因此 666 条的真实阻塞不是「缺影印」，而是「格未读出」**：需要视觉模型或人工过录已定位的 480–482 影像
  （或 CADAL 16–25 页）。tesseract 对四库孤立单字格实测输出乱码，不能充当证据。
- 「下载即可关闭」的路径：識典 `SK1599` 的「總鈐」章**已于 2026-09-12 实际抓取**（HTTP 200、767,385 B，
  原样存 `sources/excerpts/san-shi/daliuren-daquan-shidian-sk1599-zongqian.json`），
  该章在 `source-editions.json` 记为 `expected=11 actual=0` 的下载缺口**已关闭**；旧 notes「识典补本无总钤章节」是下载不全的误推。
  - ⚠️ **结构更正（B4 实测，推翻 B2 的「各 25 列」）**：11 段 = 1 标目 + 10 个表，
    但**每个表是 25 行 × 每行 1 格**（`cellType:2, rows:1, cols:1`），`table.head` 为空 ——
    **源数据根本不保留列轴**。可支持的轴是**十干**（一表一干，顺序甲乙丙丁戊己庚辛壬癸，与仓内残存表头顺序一致）；
    行角色属推断，已按推断标注。
  - 与 666 条残格互校：**唯一落位 133 / 有候选但歧义 527 / 无法落位 6**（其中 2 条是總鈐表外的神煞行，本就不该落位）。
    字元盘比值 0.9904，但两者是**不同线性化**（分会话序列相似度仅 0.08–0.24），**顺序与格址不可复原**。
  - 该源**不能**充当影印：它是二级数字化本（北大—字节跳动 → 识典），自身也带疑字；
    **666 条全部仍维持 draft，0 条升级** —— 同源转载不能投票，且 533/666 连唯一落位都达不到。
  - 唯一外部阻塞：该章扫描影印页 13–23 的签名 CDN 返回 **403**（未绕过），pageId 已记入产物待有权限时再试。
- 666 条**全部维持 draft**（找到页 ≠ 读出格），详见 `docs/closeout/B2-DALIUREN-RESIDUE-SOURCE-HUNT.md`
  与 `docs/closeout/B4-SHIDIAN-SK1599-ZONGQIAN-FETCH.md`。
