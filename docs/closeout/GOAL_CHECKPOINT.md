...
- 唯一「下载即可关闭」的路径：識典 `SK1599` 的「總鈐」章存在（11 段 = 标目 + 10 个带列结构表，各 25 列），
  与仓内残格字元盘比值 0.97，且能复原被拆散的疑字；该章在 `source-editions.json` 记为 `expected=11 actual=0`，
  **从未下载**，旧 notes「识典补本无总钤章节」是下载不全的误推。
